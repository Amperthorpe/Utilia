...
public class DrawnSymbol implements INBTSerializable<CompoundNBT>, IFunctionalObject {
    ...
    public CompoundNBT basicSerializeNBT()
    {
        ...
        nbt.putInt("face", blockFace.get3DDataValue());
    ...
    public void deserializeNBT(CompoundNBT nbt) {
        ...
        blockFace = Direction.from3DDataValue(nbt.getInt("face"));
        ...
        this.linkedInputs.forEach((input,objectSet) ->{
            if((objectSet.getB() instanceof DummyFunction))
            {
                if(objectSet.getA() instanceof DrawnSymbol)
                {
                    ...
                    {
                        ((Chunk) symbolIChunk).getCapability(Utilia.SYMBOL_CAP).ifPresent(cap->{
    ...
    public void applyServerTorque(int work, Chunk chunk)
    {
        for(PlayerEntity player : chunk.getLevel().players())
        {
            if(player.xChunk > chunk.getPos().x - SEND_WORK_RANGE && player.xChunk < chunk.getPos().x + SEND_WORK_RANGE &&
                    player.zChunk > chunk.getPos().z - SEND_WORK_RANGE && player.zChunk < chunk.getPos().z + SEND_WORK_RANGE)
            {
