...
public class SymbolSyncer
{
    ...
    public static class AddWorkMessage
    {
        ...
        public static void encode(final AddWorkMessage msg, final PacketBuffer buf)
        {
            ...
            buf.writeInt(msg.symbol.getBlockFace().get3DDataValue());
        }
        ...
        public static AddWorkMessage decode(final PacketBuffer buf)
        {
            ...
            if(Utilia.proxy.getWorld()!=null)
                symbol = new DrawnSymbol(Symbols.DEBUG, buf.readBlockPos(), Direction.from3DDataValue(buf.readInt()),Utilia.proxy.getWorld().getChunk(chunkPos.x, chunkPos.z));
        ...
        public static void handle(final AddWorkMessage msg, final Supplier<NetworkEvent.Context> contextSupplier) {
            ...
            if (context.getDirection().equals(NetworkDirection.PLAY_TO_CLIENT)) {
                context.enqueueWork(() -> {
                    World world = Utilia.proxy.getWorld();
                    Chunk chunk = world.getChunkSource().getChunk(msg.chunkPos.x, msg.chunkPos.z, true);
    ...
    public static class SymbolLinkMessage
    {
        ...
        {
            buf.writeUtf(msg.linkingFrom.getObjectType());
            buf.writeNbt(msg.linkingFrom.basicSerializeNBT());
            buf.writeUtf(msg.linkingFrom.getObjectType());
            buf.writeNbt(msg.linkingTo.basicSerializeNBT());
            buf.writeUtf(msg.input.getA());
            buf.writeUtf(msg.input.getB().name);
            buf.writeUtf(msg.outputName);
            buf.writeUtf(msg.outputType);
        }

        public static SymbolLinkMessage decode(final PacketBuffer buf)
        {
            IFunctionalObject linkingFrom = FunctionalTypeRegister.getFunctionalObject(buf.readUtf(100));
            linkingFrom.deserializeNBT(buf.readNbt());
            IFunctionalObject linkingTo = FunctionalTypeRegister.getFunctionalObject(buf.readUtf(100));
            linkingTo.deserializeNBT(buf.readNbt());

            HashableTuple<String, DataType> input = new HashableTuple<>(buf.readUtf(100),DataType.getDataType(buf.readUtf(100)));
            String outputName = buf.readUtf(100);
            String outputType = buf.readUtf(100);
        ...
        public static void handle(final SymbolLinkMessage msg, final Supplier<NetworkEvent.Context> contextSupplier) {
            ...
            if (context.getDirection().equals(NetworkDirection.PLAY_TO_SERVER)) {
                ...
                IChunk chunk = sender.level.getChunk(msg.linkingFrom.getBlockPos());
                if (chunk instanceof Chunk)
                    realLinkingFrom = msg.linkingFrom.findReal((Chunk) chunk);
                chunk = sender.level.getChunk(msg.linkingTo.getBlockPos());
                if (chunk instanceof Chunk)
                ...
                for(PlayerEntity player : ((Chunk)chunk).getLevel().players())
                {
                    ...
                    {
                        SymbolSyncer.INSTANCE.sendTo( msg, ((ServerPlayerEntity)(player)).connection.connection, NetworkDirection.PLAY_TO_CLIENT);
                    }
                }
            ...
            {
                context.setPacketHandled(true);
                PlayerEntity sender = Utilia.proxy.getPlayer();
    ...
    public static class SymbolSyncMessage
    {
        ...
        public static SymbolSyncMessage decode(final PacketBuffer buf)
        {
            ...
            {
                symbolDict.add(buf.readUtf(100));
            }
...
//                    Chunk chunk = sender.world.getChunk(msg.chunkPos.x, msg.chunkPos.z);
                    Chunk chunk = sender.level.getChunkSource().getChunk(msg.chunkPos.x, msg.chunkPos.z,true);
                    chunk.getCapability(Utilia.SYMBOL_CAP).ifPresent(symbolHandler -> {
