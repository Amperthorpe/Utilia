...
//                toFill.put(entityInput.getA(), new AtomicReference<>(entity));
                ...
                if(enabled && !(gotEntityInput && entity == null))
                {
                    ...
                    if(!world.isClientSide())
                    {
                        ...
                        TileEntity tileEntity   = world.getBlockEntity(drawnOn);
                        TileEntity tileEntityTo = world.getBlockEntity(drawnOn.offset(blockFace.getNormal()));
                        ...
                        IInventory inventory   = HopperTileEntity.getContainerAt(world,drawnOn);
                        ...
                        if(entity instanceof PlayerEntity)
                            ...
                            inventoryTo = HopperTileEntity.getContainerAt(world,drawnOn.offset(blockFace.getNormal()));
                        ...
                        if(inventory!=null && inventoryTo!=null)
                        {
                            ...
                            {
                                if (inventory.getItem(i).getCount()==0) continue;
                                ItemStack itemStack = inventory.getItem(i).copy();
                                inventory.getItem(i).shrink(1);
                                itemStack.setCount(1);
                                int prevCount = itemStack.getCount();
                                ItemStack leftover = HopperTileEntity.addItem(inventory, inventoryTo, itemStack, blockFace.getOpposite());
                        ...
                        else if(inventory!=null)
                        {
                            ...
                            for (int i=0; i<numSlots; i++)
                            {
                                ...
                                ItemStack toDrop = inventory.getItem(i).copy();
                                toDrop.setCount(1);
                                inventory.getItem(i).shrink(1);
                                ...
                                DefaultDispenseItemBehavior.spawnItem(world,toDrop,1,blockFace, dropFrom);
                                symbol.applyServerTorque(80, chunk);
                                break;
                            }
                        }
                            ...
                            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(cap ->{
                                ...
                                for (int i=0; i<numSlots; i++)
                                {
                                    if(cap.getStackInSlot(i).getCount()>0)
                                    {
                                        ...
                                        DefaultDispenseItemBehavior.spawnItem(world,item,1,blockFace, dropFrom);
                                        symbol.applyServerTorque(80, chunk);
                                        break;
                                    }
                                }
