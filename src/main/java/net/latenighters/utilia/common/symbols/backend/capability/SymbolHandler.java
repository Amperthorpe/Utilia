...
import net.latenighters.utilia.common.event.ClientChunks;
...
public class SymbolHandler implements ISymbolHandler, ICapabilitySerializable<CompoundNBT> {
    ...
    public static void onBlockBreakEvent(BlockEvent.BreakEvent event)
    {
        if(!event.getWorld().isClientSide()) {
            ...
            Chunk chunk = ((ServerWorld)event.getWorld()).getChunkSource().getChunk(chunkPos.x, chunkPos.z, true);
            assert chunk != null;
    ...
    {
        if(Utilia.proxy.getWorld() == null) return;
        ...
        if(evt.side.isClient())
        {
            ...
            for(Chunk chunk : chunks)
            {
                ...
                chunk.getCapability(Utilia.SYMBOL_CAP).ifPresent(symbolHandler ->{
                        ...
                        symbolHandler.tick(Utilia.proxy.getWorld(), chunk);
                });
            }
        }
    }
    ...
    public HashableTuple<List<HashableTuple<String, Object>>, List<HashableTuple<String, Object>>> getPrintableResolution(DrawnSymbol _symbol, Chunk chunk) {
        ...
        symbol.getOutputs().forEach(out->{
            HashableTuple<IFunctionalObject,IFunctional> key = new HashableTuple<IFunctionalObject,IFunctional>(symbol,out);
            if(previousResolution.containsKey(key)) {
                Object resolution = previousResolution.get(key);
                outputs.add(new HashableTuple<>(out.getName(), resolution));
            }
            else{
                outputs.add(new HashableTuple<>(out.getName(), "N/A"));
            }
        });
    ...
    public DrawnSymbol getSymbolAt2(BlockPos position, Direction blockFace)
    {
        ...
        {
            if (symbol.getBlockFace().get3DDataValue() == blockFace.get3DDataValue())
            {
                return symbol;
            }
        }
    ...
    public CompoundNBT serializeNBT() {
        ...
        instance.ifPresent(it ->
        {
            ...
            retVal.putInt("tag_type", symbolList.hashCode());
    ...
    public static final ResourceLocation NAME = new ResourceLocation(Utilia.MODID, "symbolhandler");
    private LazyOptional<ISymbolHandler> instance = LazyOptional.of(Utilia.SYMBOL_CAP::getDefaultInstance);

    @Nonnull
    @Override
    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
            return Utilia.SYMBOL_CAP.orEmpty(cap,instance);
    }
